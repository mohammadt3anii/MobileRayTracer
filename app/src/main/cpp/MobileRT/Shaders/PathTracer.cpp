...
PathTracer::PathTracer(RayTracer &rayTracer, const Scene &scene) :
        Shader(rayTracer, scene) {
}

void PathTracer::shade(RGB &rgb, Intersection &intersection, const Ray &ray,
    ...
    if (!intersection.material_->Kd_.isZero()) {
        ...
        for (unsigned int i(0); i < sizeLights; i++)//for each light
        {
            ...
            //calculates the vector starting in the intersection to the light
