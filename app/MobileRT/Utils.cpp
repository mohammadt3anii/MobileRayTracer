...
int ::MobileRT::roundDownToMultipleOf(const ::std::int32_t value, const ::std::int32_t multiple) noexcept {
    const ::std::int32_t rest{value % multiple};
    return rest > 1 ? value - rest : value;
}

//https://en.wikipedia.org/wiki/Halton_sequence
float ::MobileRT::haltonSequence(::std::uint32_t index, const ::std::uint32_t base) noexcept {
    float f{1.0f};
    float result{0.0f};
    while (index > 0) {
        f /= base;
...
unsigned ::MobileRT::incrementalAvg(const ::glm::vec3 sample, const ::std::uint32_t avg, const ::std::uint32_t numSample) noexcept {
    //toneMap();
