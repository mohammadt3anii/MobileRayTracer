...
<::std::uint32_t> (gridSize)
    ...
    void RegularGrid<T>::addPrimitives(::std::vector<T> &&primitives) noexcept {
        ...
        for (auto &primitive : primitives) {
            ...
            x1 = ::std::max(0, x1);
            x2 = ::std::min(gridSize_ - 1, x2);
            x2 = ::std::abs(sizeX) < Epsilon? 0 : x2;
            x1 = ::std::min(x1, x2);
            ...
            y1 = ::std::max(0, y1);
            y2 = ::std::min(gridSize_ - 1, y2);
            y2 = ::std::abs(sizeY) < Epsilon? 0 : y2;
            y1 = ::std::min(y2, y1);
            ...
            z1 = ::std::max(0, z1);
            z2 = ::std::min(gridSize_ - 1, z2);
            z2 = ::std::abs(sizeZ) < Epsilon? 0 : z2;
            z1 = ::std::min(z2, z1);
    ...
    bool RegularGrid<T>::trace(Intersection *const intersection, const Ray &ray) noexcept {
        ...
        Y = ::std::max(0, Y);
        Z = ::std::max(0, Z);
        X = ::std::min(gridSize_ - 1, X);
        Y = ::std::min(gridSize_ - 1, Y);
        Z = ::std::min(gridSize_ - 1, Z);
        ...
        if (::std::abs(ray.direction_[0]) > Epsilon) {
        ...
        if (::std::abs(ray.direction_[1]) > Epsilon) {
        ...
        if (::std::abs(ray.direction_[2]) > Epsilon) {
    ...
    template<typename T>
    bool RegularGrid<T>::shadowTrace(const Ray &ray, const float dist) noexcept {
        if (!hasPrimitives_) {
            return false;
        }
        // setup 3DDDA (double check reusability of primary ray data)
