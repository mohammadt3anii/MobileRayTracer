\newacronym{mei}{MEI}{Mestrado em Engenharia Informática}
\newacronym{miei}{MiEI}{Mestrado Integrado em Engenharia Informática}
\newacronym{di}{DI}{Departamento de Informática}
\newacronym{um}{UM}{Universidade do Minho}
\newacronym{api}{API}{Application Programming Interface}
\newacronym{gpl}{GPL}{GNU General Public License}
\newacronym{bsd}{BSD}{Berkeley Software Distribution}
\newacronym{cg}{CG}{Computer Graphics}
\newacronym{rt}{RT}{Ray Tracing}
\newacronym{foss}{FOSS}{Free and open-source software}
\newacronym{cpu}{CPU}{Central processing unit}
\newacronym{gpu}{GPU}{Graphics processing unit}
\newacronym{os}{OS}{Operating system}
\newacronym{ilp}{ILP}{Instruction-level parallelism}
\newacronym{simd}{SIMD}{Single instruction, multiple data}
\newacronym{ram}{RAM}{Random-access memory}
\newacronym{smt}{SMT}{Simultaneous multithreading}
\newacronym{stl}{STL}{Standard Template Library}
\newacronym{libstdc++}{Libstdc++}{C++ Standard Library}
\newacronym{cstdlib}{cstdlib}{C Standard Library}
\newacronym{alu}{ALU}{Arithmetic logic unit}
\newacronym{risc}{RISC}{Reduced instruction set computer}
\newacronym{cisc}{CISC}{Complex instruction set computer}
\newacronym{glm}{GLM}{OpenGL Mathematics}
\newacronym{gtest}{GTest}{Google Test}
\newacronym{sdk}{SDK}{Software development kit}
\newacronym{ndk}{NDK}{Native Development Kit}
\newacronym{jni}{JNI}{Java Native Interface}
\newacronym{opengl}{OpenGL}{Open Graphics Library}
\newacronym{jvm}{JVM}{Java virtual machine}
\newacronym{dlp}{DLP}{Data Level Parallelism}
\newacronym{gi}{GI}{Global Illumination}
\newacronym{app}{App}{Application}
\newacronym{lib}{Lib}{Library}
\newacronym{gui}{GUI}{Graphical User Interface}
\newacronym{aabb}{AABB}{Axis-Aligned Bounding Box}
\newacronym{bvh}{BVH}{Bounding volume hierarchy}
\newacronym{pixel}{Pixel}{Picture element}
\newacronym{voxel}{Voxel}{Volume element}
\newacronym{texel}{Texel}{Texture element}
\newacronym{obb}{OBB}{Oriented Bounding Box}
\newacronym{k-dop}{k-DOP}{k-Discrete Oriented Prototype}
\newacronym{sah}{SAH}{Surface Area Heuristic}
\newacronym{brdf}{BRDF}{Bidirectional Reflectance Distribution Function}
\newacronym{bsdf}{BSDF}{Bidirectional Scattering Distribution Function}
\newacronym{btdf}{BTDF}{Bidirectional Transmittance Distribution Function}
\newacronym{bssrdf}{BSSRDF}{Bidirectional Scattering-Surface Reflectance Distribution Function}
\newacronym{ui}{UI}{User Interface}
\newacronym{demo}{Demo}{Demonstration}
\newacronym{ide}{IDE}{Integrated Development Environment}
\newacronym{rgb}{RGB}{Red Green Blue}