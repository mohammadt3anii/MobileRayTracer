...
\begin{itemize}
	...
	\item Convex Hull: is the smallest convex volume containing the object.
\end{itemize}
...
It is provided the perspective and orthographic cameras, the area and point lights, 6 Samplers, 5 Shaders and 1 Object loader.
...
\par
A shader is the Rendering Component that describes how the Rendering Equation is approximated.
...
\par
There were developed 5 shaders: NoShadows, Whitted, PathTracer, DepthMap and DiffuseMaterial.
...
Then to generate a new ray direction in a world coordinates from a hemisphere it is needed to distinguish the 6 different possible materials:
...
\par
The figure \ref{Secondary_Rays.} illustrates those 6 possible scenarios.
...
The equations \ref{Diffuse_Reflection.} shows the simplification of the reflected diffuse color calculation equation.
...
\begin{figure}[H]
	\centering
	\caption{The spherical coordinates.}
...
\begin{figure}[H]
	\centering
	\caption{The possible directions in secondary rays.}
...
\begin{enumerate}
	\item std::uniform\_real\_distribution - which produces random floating-point values, uniformly distributed on the interval [a, b), that the user specifies.
	\item std::mt19937 - pseudo-random generator of 32-bit numbers with a state size of 19937 bits.
\end{enumerate}
...
\begin{lstlisting}[caption={Algorithm of getPosition in PointLight}, captionpos=b, label=getPositionPointLight]
...
\begin{lstlisting}[caption={Algorithm of intersect in PointLight}, captionpos=b, label=intersectPointLight]
return intersection
\end{lstlisting}
...
\begin{lstlisting}[caption={Algorithm of getPosition in AreaLight}, captionpos=b, label=AreaLight]
...
\begin{lstlisting}[caption={Algorithm of intersect in AreaLight}, captionpos=b, label=AreaLight]
