...
%\begin{lstlisting}[caption={Algorithm of DepthMap Shader}, captionpos=b, label=DepthMap]
%maxDist = length(maxPoint - rayOrigin) * 1.1
%depth = max((maxDist - intersectionLength) / maxDist, 0)
%rgb = {depth, depth, depth}
%\end{lstlisting}
...
\begin{algorithm}
	...
	\eIf{\var{kD} > $0$}{
		$\var{outputRGB} \leftarrow \var{kD}$ \\
	}{\eIf{\var{kS} > $0$}{
			$\var{outputRGB} \leftarrow \var{kS}$ \\
		}{\eIf{\var{kS} > $0$}{
				$\var{outputRGB} \leftarrow \var{kT}$ \\
			}{\uIf{\var{Le} > $0$}{
					$\var{outputRGB} \leftarrow \var{Le}$ \\
				}
			}
		}
	}
...
\begin{algorithm}
	...
	\ForAll{\var{LightSources}}{
		\ForAll{\var{samples}}{
			$\var{vectorToLight} \leftarrow \var{lightPos} - \var{intersectionPoint}$ \\
			$\var{cosNL} \leftarrow \var{vectorToLight} \cdot \var{intersectionNormal}$ \\
			\uIf{\var{cosNL} > $0$}{
				$\var{outputRGB} \leftarrow \var{outputRGB} + (\var{kD} * \var{radLight} * \var{cosNL})$ \\
			}
			$\var{outputRGB} \leftarrow \var{outputRGB} / \var{\#samples}$ \\
			$\var{outputRGB} \leftarrow \var{outputRGB} / \var{\#lights}$ \\
			$\var{outputRGB} \leftarrow \var{outputRGB} + \var{kD} * 0.1$ \\
		}
	}
\end{algorithm}
...
glm::vec3 refractDir = glm::refract(rayDirection, shadingNormal, 1.0 / refractiveIndice)
...
\begin{algorithm}
	...
	\Input{\var{rayDirection}, \var{shadingNormal}, \var{refractiveIndice}, \var{rayDepth}, \var{intersectionPoint}, \var{intersectionPrimitive},  \var{kT}}
	\Output{\var{outputRGB}}
	
	$\var{refractDir} \leftarrow glm::refract(\var{rayDirection}, \var{shadingNormal}, 1 / \var{refractiveIndice})$ \\
	$\var{transmissionRay} \leftarrow Ray (\var{refractDir}, \var{intersectionPoint}, \var{rayDepth} + 1, \var{intersectionPrimitive})$ \\
	$\var{LiT\_RGB} \leftarrow rayTrace (\var{transmissionRay})$ \\
	$\var{specularTransmissionColor} \leftarrow \var{kT} * \var{LiT\_RGB}$ \\
\end{algorithm}
...
\begin{lstlisting}[caption={Algorithm of Whitted Shader}, captionpos=b, label=Whitted, basicstyle=\small]
if (intersected material is diffuse) {
  for each light source
    for each sample
      vectorToLight = lightPosition - intersectionPoint
...
if (intersected material is specular refractive) {
  glm::vec3 refractDir = glm::refract(rayDirection, shadingNormal, 1 / refractiveIndice)
  Ray transmissionRay (refractDir, intersectionPoint, rayDepth + 1, intersectionPrimitive)
  glm::vec3 LiT_RGB
  rayTrace(&LiT_RGB, transmissionRay)
  rgb += kT * LiT_RGB
	...
	\uIf{intersected material is diffuse} {
		\ForAll{\var{LightSources}}{
			\ForAll{\var{samples}}{
				...
				$\var{cosNL} \leftarrow \var{vectorToLight} \cdot \var{intersectionNormal}$ \\
				\uIf{\var{cosNL} > $0$}{
					$\var{shadowRay} \leftarrow Ray(\var{intersectionPoint}, \var{vectorToLight}, \var{distanceToLight}, \var{rayDepth} + 1)$ \\
					\uIf{!shadowTrace (\var{shadowRay})}{
						$\var{outputRGB} \leftarrow \var{outputRGB} + \var{radLight} * \var{cosNL}$ \\
					}
				}
			}
		}
		$\var{outputRGB} \leftarrow \var{outputRGB} / \var{\#samples}$ \\
		$\var{outputRGB} \leftarrow \var{outputRGB} * \var{kD}$ \\
	}

	\uIf{intersected material is specular reflective} {
		$\var{reflectionDir} \leftarrow glm::reflect(\var{rayDir}, \var{shadingNormal})$ \\
		$\var{specularRay} \leftarrow Ray(\var{reflectionDir}, \var{intersectionPoint}, \var{rayDepth} + 1, \var{intersectionPrimitive})$ \\
		$\var{LiS\_RGB} \leftarrow rayTrace(\var{specularRay})$ \\
		$\var{outputRGB} \leftarrow \var{kS} * \var{LiS\_RGB})$ \\
	}
...
\begin{lstlisting}[caption={Method in Shader which generates a random direction in an hemisphere.}, captionpos=b, label=Shader, basicstyle=\small]
...
glm::vec3 u = std::abs(normal.x) > 0.1 ? glm::vec3 {0, 1, 0} : glm::vec3 {1, 0, 0};
...
\begin{algorithm}
	...
	\Input{$\vec{normal}$}
	\Output{$\vec{newDirection}$}
	
	$\var{phi} \leftarrow glm::refract(2 * \pi, \var{uniformRandom1})$ \\
	$\var{r2} \leftarrow \var{uniformRandom2}$ \\
	$\var{cosTheta} \leftarrow \sqrt{\var{r2}}$ \\
	$\vec{u} \leftarrow \left| \var{normal.x} \right| > 0? [0,1,0] : [1,0,0]$ \\
	$\vec{u} \leftarrow normalize(\vec{u} \times \vec{normal})$ \\
	$\vec{v} \leftarrow \vec{normal} \times \vec{u}$ \\
	$\vec{newDirection} \leftarrow normalize(\vec{u} * (\cos(\var{phi}) * \var{cosTheta}) + \vec{v} * (\sin(\var{phi}) * \var{cosTheta}) + \vec{normal} * \sqrt(1 - \var{r2})) $ \\
\end{algorithm}
...
\begin{lstlisting}[caption={Algorithm of Path Tracer Shader}, captionpos=b, label=PathTracer, basicstyle=\small]
if (intersected material is diffuse) {
  for each light
    for each sample
...
\begin{algorithm}
	...
	\uIf{intersected material is diffuse}{
		\ForEach{light}{
			\ForEach{sample}{
				$\var{light} \leftarrow \var{samplerLight}.getSample()$
				$\var{vectorToLight} \leftarrow \var{lightPosition} - \var{intersectionPoint}$
				$\var{cosNL} \leftarrow \var{vectorToLight} \cdot \vec{shadingNormal}$
				\uIf{$\var{cosNL} > 0$} {
					$\vec{shadowRay} \leftarrow Ray(\var{intersectionPoint}, \var{vectorToLight}, \var{distanceToLight}, \var{rayDepth} + 1)$
					\uIf{$!shadowTrace(\var{shadowRay}) > 0$} {
						$\var{outputRGB} \leftarrow \var{outputRGB} + \var{radLight} * \var{cosNL}$
					}
				}
			}
