...
\par
Last, but not least, is that the code was developed in a modular way.
...
This method also allows to choose the number of threads that will render the image to the bitmap.
The image plane is divided into 16 tiles of pixels and it is traced one primary ray per pixel in the tile.

\begin{figure}[H]
	\centering
	\caption{Illustration of tiling image plane.}
	\label{Illustration of tiling image plane.}
...
\par
Finally, the \textit{registerToneMapper} is a method that allows the user to define its own tone mapping function that needs to receive as a parameter a float number and returns that number transformed according to the defined function.
Typically, renderers based in physics generate images with high dynamic range, with luminances covering a range of 14 orders of magnitude:
$[10^{-5} .. 10^8] cd / m^2$.
But the conventional monitors typically cover a luminance range of only 2 orders of magnitude:
$[10^0 .. 10^2] cd / m^2$.
So, it is the tone mapper that maps the HDR luminance according to the monitor luminance.
...
\par
The \textit{trace}, as the name implies, is a method that tries to intersect a ray with all the primitives and light sources and returns the closest intersection to the origin of the ray.
...
The purpose of this method is to simulate the shadows in the scene.
...
So, the intersect method implemented has the next algorithm:
...
\begin{figure}[H]
	\centering
	\caption{Illustration of a ray intersecting a plane.}
	\label{Plane.}
...
\begin{figure}[H]
	\centering
	\caption{Illustration of a ray intersecting a sphere in two points.}
	\label{Sphere.}
...
\begin{figure}[H]
	\centering
	\caption{Illustration of a ray intersecting a triangle.}
	\label{Sphere.}
	\includegraphics[keepaspectratio,scale=1.0]{triangle.png}
\end{figure}

\subsection{Accelerator Structures}
