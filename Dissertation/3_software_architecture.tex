...
\begin{algorithm}
	\small
	\label{Shader}
	\caption{Algorithm in Shader which generates a random direction in an hemisphere.}
	\Input{$\vec{normal}$}
	\Output{$\vec{newDir}$}
...
\begin{algorithm}
	...
	$\var{sample} \gets \var{sample} + 1$ \tcp{atomic operation}
...
\begin{algorithm}
	...
	\caption{Algorithm of Halton Sequence}
...
\begin{algorithm}
	...
	\caption{Algorithm of MersenneTwister Sampler}
	\Input{}
	\Output{\var{res}}
	
	$\var{uniform\_dist} \gets std::uniform\_real\_distribution (0, 1)$ \\
	$\var{gen} \gets std::mt19937 (std::random\_device())$ \\
	$\var{res} \gets \var{uniform\_dist(gen)}$ \\
\end{algorithm}
...
\begin{algorithm}
	\small
	\label{getPositionPointLight}
	\caption{Algorithm of getPosition in Point light}
	\Input{}
	\Output{\var{lightPosition}}
	
	return \var{lightPosition} \\
\end{algorithm}
...
\begin{algorithm}
	\small
	\label{getPositionPointLight}
	\caption{Algorithm of intersect in Point light}
	\Input{\var{intersection}, \var{ray}}
	\Output{\var{intersection}}
	
	return \var{intersection} \\
\end{algorithm}
...
glm::vec3 position = triangle.A + R * triangle.AB + S * triangle.AC
return position
\end{lstlisting}
...
\begin{algorithm}
	\small
	\label{AreaLight}
	\caption{Algorithm of getPosition in Area light}
	\Input{}
	\Output{\var{position}}
	
	$\var{R} \gets \var{samplerPointLight}.getSample()$ \\
	$\var{S} \gets \var{samplerPointLight}.getSample()$ \\
	\uIf{$\var{R} + \var{S} \geq 1$}{
		$\var{R} \gets 1 - \var{R}$ \\
		$\var{S} \gets 1 - \var{S}$ \\
	}
	$\var{position} \gets \var{triangle}.A + \var{R} \times \var{triangle}.AB + S \times \var{triangle}.AC$ \\
\end{algorithm}
...
\begin{algorithm}
	\small
	\label{Perspective}
	\caption{Algorithm of generateRay in Perspective Camera}
	\Input{\var{u}, \var{v}, \var{deviationU}, \var{deviationV}}
	\Output{\var{ray}}
	
	$\var{rightFactor} \gets arctan(\var{hFov} \times (\var{u} - 0.5)) + \var{deviationU}$ \\
	$\vec{rightVector} \gets \var{right} * \var{rightFactor}$ \\
	$\var{upFactor} \gets arctan(\var{vFov} \times (0.5 - \var{v})) + \var{deviationV}$ \\
	$\vec{upVector} \gets \var{up} \times \var{upFactor}$ \\
	$\var{destinationPoint} \gets \var{cameraPos} + \vec{cameraDir} + \vec{rightVector} + \vec{upVector}$ \\
	$\vec{rayDir} \gets \var{destinationPoint} - \var{cameraPos}$ \\
	$\var{ray} \gets Ray(normalize(\vec{rayDir}), \var{cameraPos})$ \\
\end{algorithm}
...
\begin{algorithm}
	\small
	\label{Orthographic}
	\caption{Algorithm of generateRay in Orthographic Camera}
	\Input{\var{u}, \var{v}, \var{deviationU}, \var{deviationV}}
	\Output{\var{ray}}
	
	$\var{rightFactor} \gets (\var{u} - 0.5) \times \var{sizeH}$ \\
	$\vec{rightVector} \gets \var{right} \times \var{rightFactor} + \var{right} \times \var{deviationU}$ \\
	$\var{upFactor} \gets (0.5 - \var{v}) \times \var{sizeV}$ \\
	$\vec{upVector} \gets \var{up} \times \var{upFactor} + \var{up} \times \var{deviationV}$ \\
	$\var{rayOrigin} \gets \var{cameraPos} + \vec{rightVector} + \vec{upVector}$ \\
	$\var{ray} \gets Ray(\vec{cameraDir}, \var{rayOrigin})$ \\
\end{algorithm}
