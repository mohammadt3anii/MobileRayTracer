...
void PathTracer::shade(RGB &rgb, Intersection &intersection, Ray &ray) const {
    ...
    if (kD.isNotZero())
    {
        ...
        if (rayDepth <= RAY_DEPTH_MIN || uniform_dist(gen) > finish_probability)
        {
            ...
            const float localMagnitude(std::sqrt(localX*localX + localY*localY + localZ*localZ));
            ...
            //Create a Local Coordinate System Oriented along the Shading Normal
            Vector3D Nt;
            Vector3D Nb;
            createCoordinateSystem(shadingNormal, Nt, Nb);

            float globalX2(localX * Nb.x_ + localY * shadingNormal.x_ + localZ * Nt.x_);
            float globalY2(localX * Nb.y_ + localY * shadingNormal.y_ + localZ * Nt.y_);
            float globalZ2(localX * Nb.z_ + localY * shadingNormal.z_ + localZ * Nt.z_);

            float globalMagnitude2 (std::sqrt(
                globalX2*globalX2 + globalY2*globalY2 + globalZ2*globalZ2));
            globalX2 /= globalMagnitude2;
            globalY2 /= globalMagnitude2;
            globalZ2 /= globalMagnitude2;
            ...
            Ray normalizedSecundaryRay (globalX, globalY, globalZ, intersection.point_, rayDepth + 1u);
            ...
            if (secundaryIntersection.material_->Le_.isNotZero()) LiD.reset();
...
void PathTracer::createCoordinateSystem(const Vector3D &normal,
                            Vector3D &perpendicularToNormal,
                            Vector3D &perpendicular) const
{
    if (std::fabs(normal.x_) > std::fabs(normal.y_)) {
        perpendicularToNormal = Vector3D(normal.z_, 0.0f, -normal.x_);
        perpendicularToNormal.normalize();
    }
    else {
        perpendicularToNormal = Vector3D(0.0f, -normal.z_, normal.y_);
        perpendicularToNormal.normalize();
    }
    perpendicular = normal.crossProduct(perpendicularToNormal);
}
