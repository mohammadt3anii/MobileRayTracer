...
namespace MobileRT {
  ...
  class BVH2 final {
    ...
    private:
      AABB build(::std::vector<MobileRT::Primitive<T>> &&primitives, const unsigned depth,
    ...
    public:
      ...
      explicit BVH2<T>(::std::vector<MobileRT::Primitive<T>> &&primitives, const unsigned depth = 0,
                       const uint32_t currentId = 0) noexcept {
      ...
      bool trace(::MobileRT::Intersection *const intersection, const ::MobileRT::Ray &ray,
                 const unsigned depth = 0, const uint32_t currentNodeId = 0) noexcept {
      ...
      bool shadowTrace(::MobileRT::Intersection *const intersection, const ::MobileRT::Ray &ray,
                       const unsigned depth = 0, const uint32_t currentNodeId = 0) noexcept {
        if (boxes_.at(currentNodeId).intersect(ray)) {
          //node at the bottom of tree - no childs
            const uint32_t aux{static_cast<uint32_t>(1 << depth)};
