...
void RegularGrid::addPrimitives
  ...
  for (T &primitive : primitives) {
    ...
    auto x1(static_cast<int>((bv1.x_ - m_Extends.pointMin_.x_) * dx_reci));
    auto x2(static_cast<int>((bv2.x_ - m_Extends.pointMin_.x_) * dx_reci) + 1);
    x1 = (x1 < 0) ? 0 : x1;
    x2 = (x2 > (gridSize_ - 1)) ? gridSize_ - 1 : x2;
    auto y1(static_cast<int>((bv1.y_ - m_Extends.pointMin_.y_) * dy_reci));
    auto y2(static_cast<int>((bv2.y_ - m_Extends.pointMin_.y_) * dy_reci) + 1);
    y1 = (y1 < 0) ? 0 : y1;
    y2 = (y2 > (gridSize_ - 1)) ? gridSize_ - 1 : y2;
    auto z1(static_cast<int>((bv1.z_ - m_Extends.pointMin_.z_) * dz_reci));
    auto z2(static_cast<int>((bv2.z_ - m_Extends.pointMin_.z_) * dz_reci) + 1);
    z1 = (z1 < 0) ? 0 : z1;
